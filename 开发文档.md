...
WhatTheDogDoing 是一个前后端分离的即时通讯系统，面向普通用户提供注册登录、好友管理、单聊、群聊、消息收发、多媒体消息、群公告、群成员管理、实时通知、个人笔记、好友动态、消息转译和群聊机器人等能力。
...
当前版本采用 Aegis 轻骑士世界观包装产品体验：主界面以暖暗铜金、玻璃质感、羊皮纸式内容区和骑士团头像为视觉基调；群聊助手名为“露恩”，右键消息功能中的 AI 能力命名为“转译”。这些设定只影响展示和交互文案，不改变即时通讯系统的核心数据模型。
...
| 好友动态 | 好友可发布动态、上传图片、点赞、评论；仅自己和好友可见。 | `/api/chat/moments*` |
