...
Aegis 是一个前后端分离的即时通讯系统，面向普通用户提供注册登录、好友管理、单聊、群聊、消息收发、多媒体消息、群公告、群成员管理、实时通知、个人笔记、广场动态、消息转译和群聊机器人等能力。
...
当前版本采用 Aegis 轻骑士世界观包装产品体验：主界面以暖暗铜金、玻璃质感、羊皮纸式内容区和骑士团头像为视觉基调；群聊助手名为“露恩”，右键消息功能中的 AI 能力命名为“转译”。课程小组名称仍为 WhatTheDogDoing，但项目产品名和界面品牌统一使用 Aegis。这些设定只影响展示和交互文案，不改变即时通讯系统的核心数据模型。
...
本节按《软工大作业 - 即时通讯系统 需求文档.pdf》的功能评分项进行核对，便于验收时快速定位实现位置。

| 要求 | 当前实现状态 | 主要实现位置 |
| --- | --- | --- |
| PC Web 端正常显示 | 已实现。React/Vite 单页应用，聊天主界面首屏即为会话列表和消息区。 | `frontend/src/App.jsx`、`frontend/src/components/stage2/` |
| 注册、登录、登出 | 已实现。密码使用 bcrypt 哈希，登录返回 JWT。 | `backend/app/auth.py`、`frontend/src/services/api.js` |
| 注销账号 | 已实现。删除当前用户并清理好友关系、私聊、置顶、消息和会话成员记录。 | `backend/app/main.py` |
| 个人信息编辑 | 已实现。支持昵称、头像、邮箱、手机号、密码等，敏感信息修改需要密码验证。 | `backend/app/auth.py` |
| 好友搜索与申请 | 已实现。可搜索用户、发送申请、查看收到和发出的申请、接受或拒绝。 | `backend/app/chat.py` |
| 好友删除、备注、分组 | 已实现。备注和分组按当前用户视角保存。 | `backend/app/chat.py`、`frontend/src/components/stage2/Overlays.jsx` |
| 私聊 | 已实现。好友建立后创建或复用两人会话。 | `backend/app/chat.py` |
| 群聊 | 已实现。支持创建群、邀请成员、群成员列表、群昵称。 | `backend/app/chat.py` |
| 未读消息 | 已实现。会话成员保存 `read_index`，会话列表计算 badge；免打扰会话不显示普通未读角标。 | `backend/app/chat.py`、`frontend/src/App.jsx` |
| 消息回复 | 已实现。消息保存 `reply_to_id` 并在前端展示回复引用。 | `backend/app/chat.py`、`frontend/src/components/stage2/ChatMainView.jsx` |
| 聊天记录搜索 | 已实现。前端在当前会话消息中搜索并定位。 | `frontend/src/components/stage2/Overlays.jsx` |
| 删除/撤回消息 | 已实现。只能撤回本人消息，并向会话成员推送更新。 | `backend/app/chat.py` |
| 置顶与免打扰 | 已实现。置顶表和成员免打扰字段按用户维度生效。 | `backend/app/models.py`、`backend/app/chat.py` |
| 群成员管理 | 已实现。群主和管理员有不同权限，成员详情展示真实头像。 | `backend/app/chat.py`、`frontend/src/components/stage2/Overlays.jsx` |
| 群公告 | 已实现。群主/管理员发布，成员收到 Aegis 风格弹层并可确认。 | `backend/app/chat.py`、`frontend/src/components/stage2/ChatMainView.jsx` |
| 邀请审核 | 已实现。普通成员可发起入群申请，群主/管理员审核。 | `backend/app/chat.py` |
| 退出/解散群 | 已实现。普通成员可退出，群主可解散；群主退出前需转让。 | `backend/app/chat.py` |
| 实时性 | 已实现。WebSocket 推送为主，HTTP 长轮询为降级通道。 | `backend/app/chat.py` |
| 数据完整性 | 已实现。消息持久化、已读位置、成员校验、文件大小限制和级联清理共同保证记录完整。 | `backend/app/models.py`、`backend/app/chat.py` |

## 9. UI 与世界观设计

当前前端不再沿用早期绿色即时通讯模板，而是统一为 Aegis 轻骑士视觉系统：

| 设计项 | 当前约定 |
| --- | --- |
| 色彩 | 暖暗铜金为主，辅以象牙白、石灰米色、低饱和暗棕；危险操作仍保留红色语义。 |
| 图形 | 使用骑士团头像、群徽、玻璃面板、轻微纸质纹理和铜金按钮。 |
| 交互命名 | AI 消息翻译称为“转译”；群聊机器人称为“露恩”。 |
| 弹层 | 群公告、好友申请、成员管理、转发确认等弹层由 `10-aegis-theme.css` 统一覆盖。 |
| 头像 | 用户上传头像、预设头像和群头像都按 `cover` 显示；缺省值由 `resolveAegisAvatar` 兜底为 Aegis 头像。 |

视觉覆盖集中在 `frontend/src/styles/app/10-aegis-theme.css`，该文件位于样式引入链末尾，用于压住早期 CSS 中残留的绿色按钮、状态点和模板化弹窗。

## 10. 扩展功能与维护说明

除作业基础要求外，当前版本还包含以下扩展能力：

| 功能 | 说明 | 主要接口 |
| --- | --- | --- |
| 消息转译 | 对文本或机器人消息调用 AI 网关返回目标语言译文。 | `POST /api/chat/messages/{message_id}/translate` |
| 群聊机器人露恩 | 群聊中提及露恩时，后端根据最近上下文生成机器人回复。 | `backend/app/ai_gateway.py`、`backend/app/chat.py` |
| 动态广场 | 全局可见，登录用户可发布动态、上传图片、点赞、评论。 | `/api/chat/moments*` |
| 个人笔记 | 用户可创建、编辑、删除个人笔记。 | `/api/chat/notes*` |
| 井字棋邀请 | 私聊中可邀请好友进行井字棋小游戏。 | `/api/chat/games/tictactoe*` |
| 收藏消息 | 前端维护消息收藏入口，便于回到原会话定位。 | `frontend/src/App.jsx` |

AI 网关通过环境变量或本机配置文件读取服务配置，仓库中不应提交真实 API Key；测试代码只允许使用无效占位值。

## 11. 测试与质量保障

后端测试位于 `backend/tests/`，覆盖认证、好友、群聊、公告确认、成员头像、实时通知、长轮询、数据库初始化迁移和 AI 网关异常映射等场景。当前建议的验收命令如下：
...
最近一次本地验证中，后端全量测试 117 个用例通过；关键文件覆盖率为 `ai_gateway.py 98%`、`chat.py 83%`、`main.py 88%`、`models.py 100%`，满足过程细则中后端覆盖率不低于 80% 的要求。

## 12. 交付与过程规范

《大作业过程分细则.pdf》要求阶段性交付功能在主分支保持可用、CI/CD 正常、后端单测覆盖率不低于 80%，并避免不规范 commit、分支和超大提交。项目维护时建议遵守以下规则：

| 事项 | 约定 |
| --- | --- |
| Commit | 使用英文 commit message，单个 commit 控制在 500 行以内。 |
| 分支 | 功能开发走独立分支，避免直接修改主分支。 |
| CI | 提交前至少运行后端测试、前端 lint 和前端 build。 |
| 配置 | 数据库 URL、AI Key 等敏感信息只通过环境变量或本机配置提供。 |
| 文档 | 新增 API、表结构、重要交互或世界观变化时同步更新本文档。 |
